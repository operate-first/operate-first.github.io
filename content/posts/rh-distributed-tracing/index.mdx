...
A [`Hello OpenTelemetry`](https://sample-app-otel-dev.apps.smaug.na.operate-first.cloud/hello) page and a [`counter`](https://sample-app-otel-dev.apps.smaug.na.operate-first.cloud/count).
...
View the route from the application running in Operate First at [https://jaegerex-otel-dev.apps.smaug.na.operate-first.cloud/](https://jaegerex-otel-dev.apps.smaug.na.operate-first.cloud/).
You can generate trace data by visiting the [`/hello`](https://sample-app-otel-dev.apps.smaug.na.operate-first.cloud/hello) and [`/count`](https://sample-app-otel-dev.apps.smaug.na.operate-first.cloud/count) pages.
...
To view logs from the application that include span_id & trace_id information:

```bash
oc get logs -n otel-dev [sample-app-podname]
```

![Logs Injected with Trace Data](logs-with-traces.png)
